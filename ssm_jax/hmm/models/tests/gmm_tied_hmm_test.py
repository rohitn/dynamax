...
from ssm_jax.hmm.models import TiedGaussianMixtureHMM
from ssm_jax.utils import monotonically_increasing


def test_random_initialization(key=jr.PRNGKey(0), num_states=3, num_mix=2, emission_dim=2):
    hmm = TiedGaussianMixtureHMM.random_initialization(key, num_states, num_mix, emission_dim)
    ...
    assert hmm.emission_covariance_matrix.value.shape == (num_states, emission_dim, emission_dim)


def test_sample(key=jr.PRNGKey(0), num_states=3, num_mix=2, emission_dim=2, num_timesteps=100):
    init_key, sample_key = jr.split(key)
    h = TiedGaussianMixtureHMM.random_initialization(init_key, num_states, num_mix, emission_dim)
...
def test_fit(key=jr.PRNGKey(0), num_states=3, num_mix=2, emission_dim=2, n_samples=1000):
    key0, key1, key2 = jr.split(key, 3)
    true_hmm = TiedGaussianMixtureHMM.random_initialization(key0, num_states, num_mix, emission_dim)
    _, emissions = true_hmm.sample(key2, n_samples)

    # Mess up the parameters and see if we can re-learn them.
    hmm = TiedGaussianMixtureHMM.random_initialization(key1, num_states, num_mix, emission_dim)

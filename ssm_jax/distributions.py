...
class MatrixNormalPrecision(tfd.TransformedDistribution):
    ...
    def __init__(self, loc, row_covariance, col_precision):
        ...
        super().__init__(
            tfd.MultivariateNormalFullCovariance(self._vec_mean, self._vec_cov),
...
def niw_posterior_update(niw_prior, sufficient_stats):
    ...
    df_pos = df_pri + N
    scale_pos = (scale_pri + SxxT + precision_pri * jnp.outer(loc_pri, loc_pri) -
                 precision_pos * jnp.outer(loc_pos, loc_pos))
...
def iw_posterior_update(iw_prior, sufficient_stats):
    ...
    return InverseWishart(df_pos, scale_pos)


class ScaledInverseChi2(tfd.TransformedDistribution):

    def __init__(self, df, scale):
        self._df = df
        self._scale = scale

        super().__init__(distribution=tfd.Chi2(df), bijector=tfb.Chain([tfb.Scale(scale * df), tfb.Reciprocal()]))
    ...
    def _parameter_properties(self, dtype, num_classes=None):
        return dict(
            ...
            # the `ParameterProperties` documentation for details.
            df=tfp.util.ParameterProperties(event_ndims=0),
            scale=tfp.util.ParameterProperties(event_ndims=0),
        )
    ...
    def _mean(self):
        assert self.df > 2, "Mean only exists if df > 2"
        return self.scale * self.df / (self.df - 2)

    def _mode(self):
        return self.scale * self.df / (self.df + 2)

    def _variance(self):
        assert self.df > 4, "Variance only exists if df > 4"
        return 2 * self.scale**2 * self.df**2 / ((self.df - 2)**2 * (self.df - 4))


class NormalInverseChi2(tfd.JointDistributionSequential):

    def __init__(self, loc, mean_concentration, df, scale):
        """
        A normal inverse chi-squared (NIX) distribution.
        """
        self._loc = loc
        self._mean_concentration = mean_concentration
        self._df = df
    ...
    def _mode(self):
        ...
        The optimal mean is :math:`\mu^* = \mu_0`. Substituting this in,
        .. math::
            p(\mu^*, \Sigma) \propto IX(\Sigma | \nu_0 + 1, \Psi_0)
        and the mode of this inverse chi-squared distribution is at
        .. math::
            \Sigma^* = \Psi_0 \times (\nu_0 + 1) / (\nu_0 + 3)
        """
        return self._scale * (self._df + 1) / (self._df + 3), self._loc
